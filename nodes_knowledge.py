"""Wissens-Nodes f??r LangGraph v2."""
...
def knowledge_router_node(state: SealAIState, *_args, **_kwargs) -> Dict[str, object]:
    ...
    classification = run_llm(
        ...
        system="Du bist ein Klassifizierer f??r Dichtungstechnik-Fragen.",
        temperature=0.0,
        max_tokens=10,
...
def knowledge_material_node(state: SealAIState, *_args, **_kwargs) -> Dict[str, object]:
    """
    Wissens-Node f??r Dichtungswerkstoffe (NBR, FKM, EPDM etc.).
    ...
    **Best Practice Nov 2025**: RAG-Augmentation vor LLM-Call.
    ...
    import sys
    print(f"[NODE_DEBUG] knowledge_material_node entered. User text: {user_text}", file=sys.stderr, flush=True)
    rag_context = search_knowledge_base.invoke({
        "query": user_text,
        "category": "materials",
        "k": 3,
        "tenant": state.tenant_id,
        "is_privileged": bool(state.is_privileged),
    })
    print(f"[NODE_DEBUG] RAG invocation complete. Context len: {len(str(rag_context))}", file=sys.stderr, flush=True)
...
def knowledge_lifetime_node(state: SealAIState, *_args, **_kwargs) -> Dict[str, object]:
    """
    Wissens-Node f??r Lebensdauer/Standzeit von Dichtungen.
...
Frage: {user_text or 'Erkl??re Einflussfaktoren auf Lebensdauer und Standzeit von Dichtungen.'}
...
Beantworte basierend auf dem Kontext."""
    ...
    reply_text = run_llm(
        ...
        prompt=prompt,
        system=(
            "Du bist ein Fachberater f??r Lebensdauer/Standzeit von Dichtungen. "
            "Nutze die Wissensdatenbank. "
            "Erkl??re typische Ausfallmechanismen (z.B. Extrusion, Verschlei??, chemischer Angriff) "
            "und die wichtigsten Einflussgr????en (Medium, Temperatur, Druck, Bewegung, Montage, Oberfl??chen). "
...
def generic_sealing_qa_node(state: SealAIState, *_args, **_kwargs) -> Dict[str, object]:
    ...
    **Best Practice Nov 2025**: RAG-Augmentation.
    ...
    # RAG-Retrieval (ohne category-Filter f??r breitere Suche)
    rag_context = search_knowledge_base.invoke({
