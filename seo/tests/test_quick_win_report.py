...
def test_quick_win_report_sorts_candidates(tmp_path):
    ...
    for query, impressions, position in [("Dichtungstechnik", 500, 7), ("PTFE-RWDR", 150, 15)]:
        upsert_page_query(conn, {"data_date": end.isoformat(), "site_url": site, "search_type": "web", "country": "ALL", "device": "ALL", "page": "https://sealai.net/wissen/gleitringdichtung-grundlagen", "query": query, "query_sanitized": query, "query_taint": QUERY_TAINT, "clicks": 1, "impressions": impressions, "ctr": 1 / impressions, "position": position, "data_state": "final", "ingested_at_utc": "now", "source": "gsc_api"})
