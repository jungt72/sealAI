...
import { buildRwdrBriefBackendPath } from "@/lib/bff/workspace";
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetchBackend(buildRwdrBriefBackendPath(), request, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        raw_inquiry: callerBody.raw_inquiry || "",
        fields: Array.isArray(callerBody.fields) ? callerBody.fields : [],
      }),
    });
