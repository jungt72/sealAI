...
test("mapWorkspaceView normalizes legacy workspace sections", () => {
  ...
  expect(workspace.caseId).toBe("case-123");
  expect(workspace.requestType).toBe("retrofit");
  expect(workspace.engineeringPath).toBe("rwdr");
  expect(workspace.decisionUnderstanding?.caseSummary).toBe("Steam RWDR case with open surface finish.");
  expect(workspace.decisionUnderstanding?.understoodNow).toEqual(["Medium: Steam"]);
  expect(workspace.decisionUnderstanding?.nextBestQuestions[0]?.reason).toBe("Surface finish drives seal lip review.");
  expect(workspace.sealApplicationProfile?.sealType).toBe("radial_shaft_seal");
  expect(workspace.mediumContext.validationStatus).toBe("documented");
  expect(workspace.cockpit?.requestType).toBe("retrofit");
  expect(workspace.cockpit?.path).toBe("rwdr");
  expect(workspace.cockpit?.sections.application_function.properties[0]?.origin).toBe("user_override");
  expect(workspace.cockpit?.sections.application_function.properties[0]?.sourceType).toBe("user_stated");
  expect(workspace.cockpit?.sections.application_function.properties[0]?.validationStatus).toBe("user_stated");
  ...
  expect(workspace.cockpit?.readiness.missingMandatoryKeys).toEqual(["speed_rpm"]);
  expect(workspace.governance.releaseStatus).toBe("manufacturer_validation_required");
  expect(workspace.governance.releaseClass).toBe("C");
  expect(workspace.completeness.coveragePercent).toBe(65);
  expect(workspace.matching.items.length).toBe(1);
  expect(workspace.matching.shortlistReady).toBe(true);
  expect(workspace.matching.inquiryReady).toBe(false);
  expect(workspace.matching.blockingReasons).toEqual(["manufacturer_validation_required"]);
  expect(workspace.matching.manufacturerFitMatrix?.status).toBe("fit_computed");
  expect(workspace.matching.manufacturerFitMatrix?.rows[0]?.manufacturerId).toBe("partner-a");
  expect(workspace.matching.manufacturerFitMatrix?.rows[0]?.fitScore).toBe(93.5);
  expect(workspace.matching.manufacturerFitMatrix?.rows[0]?.verificationLevel).toBe("documented");
  expect(workspace.rfq.documentUrl).toBeNull();
  expect(workspace.communication?.conversationPhase).toBe("clarification");
  expect(workspace.communication?.primaryQuestion).toBe("Können Sie den Betriebsdruck noch einordnen?");
  expect(workspace.parameters?.shaft_diameter_mm).toBe(50);
  expect(workspace.parameters?.speed_rpm).toBe(6000);
  expect(workspace.technicalDerivations?.[0]?.calcType).toBe("rwdr");
  expect(workspace.technicalDerivations?.[0]?.vSurfaceMPerS).toBe(3.93);
  expect(workspace.technicalDerivations?.[0]?.temperatureHeadroomC).toBe(140);
  expect(workspace.deepDiveTabs[0]?.tabId).toBe("material");
  expect(workspace.deepDiveTabs[0]?.cards[0]?.title).toBe("Werkstoffbasis");
  expect(workspace.deepDiveTabs[0]?.nextAction).toBe("Oberflaeche klaeren");
  expect(workspace.evidence.evidencePresent).toBe(true);
