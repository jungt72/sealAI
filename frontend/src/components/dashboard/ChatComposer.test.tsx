...
describe("ChatComposer", () => {
  it("renders the composer controls", () => {
    ...
    expect(screen.queryByRole("button", { name: "Antwortlänge" })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Spracheingabe" })).not.toBeInTheDocument();
  });
  ...
  it("syncs external draft values without clearing local edits on null", async () => {
    ...
    const composer = screen.getByPlaceholderText(/Anschlussfrage stellen/i);
