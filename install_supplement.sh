#!/usr/bin/env bash
# SeaLAI — Install SSoT Supplement v1 on the server
# Usage: bash install_supplement.sh
# Run from the sealai repo root (expects konzept/ directory to exist)

set -euo pipefail

TARGET_DIR="konzept"
TARGET_FILE="$TARGET_DIR/sealai_ssot_supplement_v1.md"

if [ ! -d "$TARGET_DIR" ]; then
    echo "ERROR: $TARGET_DIR directory not found. Run this from the sealai repo root." >&2
    exit 1
fi

if [ -f "$TARGET_FILE" ]; then
    echo "WARNING: $TARGET_FILE already exists. Backup as ${TARGET_FILE}.bak"
    cp "$TARGET_FILE" "${TARGET_FILE}.bak"
fi

cat > "$TARGET_FILE" << 'SUPPLEMENT_EOF'
...
if [ -s "$TARGET_FILE" ]; then
    ...
    echo ""
    echo "SUCCESS: Supplement installed at $TARGET_FILE"
    echo "  Lines: $LINES"
    echo "  Bytes: $BYTES"
    echo ""
    echo "Expected: ~776 lines, ~35 KB"
    echo ""
    if [ "$LINES" -lt 700 ]; then
        echo "WARNING: line count looks low — verify the file contents." >&2
    fi
else
    echo "ERROR: File write failed or file is empty" >&2
    exit 1
fi
