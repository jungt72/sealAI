...
<!-- template: login-username.ftl -->
    ...
    <#if section = "header">
        ${msg("sealaiUsernameTitle")}
    <#elseif section = "form">
        <p class="sealai-b2b-lead">${msg("sealaiUsernameLead")}</p>
        <div id="kc-form">
            <div id="kc-form-wrapper">
                <#if realm.password>
                    <form id="kc-form-login" class="${properties.kcFormClass!}" onsubmit="login.disabled = true; return true;" action="${url.loginAction}"
                        ...
                        <@buttons.actionGroup>
                            <@buttons.button id="kc-login" name="login" label="sealaiContinue" class=["kcButtonPrimaryClass", "kcButtonBlockClass"] />
                        </@buttons.actionGroup>
                    </form>
                </#if>
            </div>
