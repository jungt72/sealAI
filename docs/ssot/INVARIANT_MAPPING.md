...
| P2 No technical claim without status | typed claim lifecycle; claims without independent current human review quarantined; 79 owner-approved claims bound to exact authority fingerprints; source-less traps block-only; unsourced matrix inactive; MAT-GOV-01 typed result is default-off and structurally forbids positive statements | knowledge, L3, matrix, material-constraint contract, migration, and owner-review artifact tests | implemented; H1 behavior gate M15 and material-rule evidence gates remain open |
| P3 Unknown is a domain state | `core/case_state.py`, coverage contracts, typed `NeedStatus`/`PendingQuestion` in `core/interview/`, orthogonal MAT-GOV-01 input/cardinality/relation/evaluation states; MAT-GOV-01 evaluates only single-medium input and blocks every multiple cardinality, including reserved `resolved` relations, until MED-NORM-01 provides structured media | case-state, coverage, adaptive-interview, exhaustive material-state invariants, and canonical-completeness tests | partial; MAT-GOV-02 scope/null/unknown precedence and MED-NORM-01 remain open |
...
| P12 Every scope extension needs an eval gate | config flags, release gate, default-off `material_constraints_enabled`, complete unbounded canonical material evaluation, and separately bounded Legacy-Gegencheck projection | deploy-gate, flag-default, flag-off serialization, eight-condition completeness, and legacy-projection tests | remediation required; MAT-GOV-02/03, MED-NORM-01, and activation evidence open |
