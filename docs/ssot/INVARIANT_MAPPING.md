...
| P3 Unknown is a domain state | `core/case_state.py`, coverage contracts, default-off typed `NeedStatus`/`PendingQuestion` in `core/interview/` | case-state, coverage, and adaptive-interview invariant tests | partial; rwdr.v1 shadow implemented |
...
| G8 Scope | runtime maturity endpoint plus fail-closed knowledge, fit, handoff, and default-off `rwdr.v1` interview scope gate | partial | collect rwdr.v1 shadow evidence; migrate every remaining scope flag into the shared activation registry |
