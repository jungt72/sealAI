...
| `scope.media[0]` |  | Exactly one canonical `med_*` ID for a rule claim. A v2 MED-NORM identity claim instead uses exactly one scalar `media_ref`. |
...
| Evidence scope for identity claim | `MEDIA_IDENTITY_V2_REQUIRED` | `RP001-OD-01` requires the implemented typed v2 `media_identity` scope with no materials and exactly one `media_ref`; import remains blocked until real verified-human review and approval are complete. |
