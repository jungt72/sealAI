...
def get_connection_pool() -> Any:
    ...
    if _pool is None:
        ...
        conninfo = (
            settings.postgres_dsn or settings.POSTGRES_SYNC_URL or settings.database_url
        )
...
class AsyncPostgresStore(BaseStore):
    def __init__(self, pool: Any):
        ...
        self._sync_executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="store-sync"
        )
    ...
    def search(
        ...
        *,
        ...
        return self._run_sync(
            self.asearch(namespace_prefix, filter=filter, limit=limit, offset=offset)
        )
        ...
        *,
        ...
        return self._run_sync(
            self.alist_namespaces(
                prefix=prefix, suffix=suffix, limit=limit, offset=offset
            )
        )
            ...
            async with conn.cursor() as cur:
                ...
                async for row in cur:
                    ...
                    if prefix and not all(
                        ns[i] == prefix[i] for i in range(len(prefix))
                    ):
                        continue
                    if suffix and not all(
                        ns[-(len(suffix) - i)] == suffix[i] for i in range(len(suffix))
                    ):
                        continue
                    namespaces.add(ns)
