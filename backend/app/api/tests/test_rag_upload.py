...
async def test_rag_upload_same_sha_different_tenant(
    ...
    user_a = RequestUser(user_id="tenant-a", tenant_id="tenant-a", username="user", sub="tenant-a", roles=["admin"])
    user_b = RequestUser(user_id="tenant-b", tenant_id="tenant-b", username="user", sub="tenant-b", roles=["admin"])
...
async def test_rag_upload_retry_failed(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    user = RequestUser(user_id=tenant_id, tenant_id=tenant_id, username="user", sub=tenant_id, roles=["admin"])
    file_obj = DummyUploadFile(filename="doc.txt", data=b"hello")
    payload = await rag_endpoint.upload_rag_document(
        file=file_obj,
        category=None,
