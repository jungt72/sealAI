...
def test_param_patch_promotes_and_clears_staged_candidates(monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    monkeypatch.setenv("LANGGRAPH_V2_CHECKPOINTER", "memory")
    ...
    assert state_response["working_profile"]["extracted_params"]["medium"] == "steam"
    assert state_response["working_profile"]["extracted_params"]["pressure_bar"] == 25
...
def test_new_assertion_invalidates_old_derived_artifacts(monkeypatch):
    ...
    assert system_state["answer_contract"] is None
