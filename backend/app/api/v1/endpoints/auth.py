...
def login_redirect(
    state: str | None = Query(
        default=None, description="Optional OIDC state passthrough"
    ),
    nonce: str | None = Query(
        default=None, description="Optional OIDC nonce passthrough"
    ),
):
    """
    ...
    keycloak_base_url = (
        f"{settings.keycloak_issuer.rstrip('/')}/protocol/openid-connect/auth"
    )
