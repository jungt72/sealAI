...
def test_ws_state_applies_public_visibility_filter(monkeypatch: pytest.MonkeyPatch) -> None:
    captured = {}

    def fake_retrieve(**kwargs):
        captured.update(kwargs)
        return [
            {"text": "hit", "metadata": {"document_id": "doc-1"}, "vector_score": 0.5}
        ], {"k_returned": 1, "top_scores": [0.5]}

    monkeypatch.setattr(rag_tool, "hybrid_retrieve", fake_retrieve)
    monkeypatch.setattr(nodes_flows, "search_knowledge_base", rag_tool.search_knowledge_base)

    state = chat_ws._build_ws_initial_state(
        tenant_id="tenant-1",
        user_id="user-1",
        chat_id="chat-1",
        user_input="Bitte Normen prüfen",
        history=[],
        system_prompt="",
        params_patch=None,
        is_privileged=False,
