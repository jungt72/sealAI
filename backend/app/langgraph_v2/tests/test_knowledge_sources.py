...
from app.langgraph_v2.state.sealai_state import SealAIState, Intent, Source
from app.langgraph_v2.nodes.nodes_knowledge import knowledge_material_node, generic_sealing_qa_node
...
def test_knowledge_node_populates_sources(monkeypatch):
    # Mock search_knowledge_base to return hits with sources
    mock_rets = {
    ...
    # Patch the tool object itself in the nodes_knowledge module
    ...
    state = SealAIState(
        messages=[HumanMessage(content="Tell me about NBR")],
        tenant_id="tenant-1"
    )
...
def test_knowledge_node_missing_sources(monkeypatch):
    # Mock search_knowledge_base to return no hits
    mock_rets = {
    ...
    state = SealAIState(
        messages=[HumanMessage(content="Unknown item")],
        tenant_id="tenant-1"
    )
...
def test_frontdoor_triggers_rag_for_knowledge(monkeypatch):
    # Mock LLM and other frontdoor dependencies
    mock_run_llm = MagicMock(return_value='{"intent": {"goal": "explanation_or_comparison", "key": "knowledge_material"}, "frontdoor_reply": "Ok"}')
    ...
    state = SealAIState(
        messages=[HumanMessage(content="Kyrolon 79X")]
    )
    
    patch = frontdoor_discovery_node(state)
    
    assert patch.get("requires_rag") is True
    assert patch.get("needs_sources") is False

def test_frontdoor_triggers_rag_for_generic_qa(monkeypatch):
    # Mock LLM and other frontdoor dependencies
    monkeypatch.setattr("app.langgraph_v2.nodes.nodes_frontdoor.run_llm", 
...
def test_frontdoor_sources_request_forces_rag(monkeypatch):
    ...
    state = SealAIState(
        messages=[HumanMessage(content="Kyrolon 79X - Quellen (filename, sha256) bitte.")],
    )
    ...
    assert patch["intent"].goal not in ("smalltalk", "out_of_scope")
