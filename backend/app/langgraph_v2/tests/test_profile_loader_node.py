...
def test_profile_loader_uses_aget_when_available() -> None:
    ...
    store = AsyncStore()
    result = asyncio.run(profile_loader_node(SealAIState(user_id="user-1"), store))
    assert store.calls == [(("user_prefs", "user-1"), "technical_context")]
    assert result["user_context"] == {"preferred_medium": "water"}
    assert result["working_memory"].design_notes["user_profile"] == {"preferred_medium": "water"}
...
def test_profile_loader_falls_back_to_sync_get_without_aget() -> None:
    ...
    result = asyncio.run(profile_loader_node(SealAIState(user_id="user-2"), store))
    assert store.calls == [(("user_prefs", "user-2"), "technical_context")]
    assert result["user_context"] == {"preferred_medium": "oil"}
    assert result["working_memory"].design_notes["user_profile"] == {"preferred_medium": "oil"}
...
def test_profile_loader_preserves_auth_scopes_when_loading_profile() -> None:
    ...
    assert result["user_context"]["preferred_medium"] == "water"
    assert result["user_context"]["auth_scopes"] == ["openid", "mcp:knowledge:read"]
...
def test_profile_loader_resolves_user_id_from_context_metadata() -> None:
    ...
    store = AsyncStore()
    state = SealAIState(
        user_context={"metadata": {"user_id": "ctx-user-7"}},
    )
    result = asyncio.run(profile_loader_node(state, store))
    assert store.calls == [(("user_prefs", "ctx-user-7"), "technical_context")]
    assert result["working_memory"].design_notes["user_domains"] == ["hydrogen"]
    assert result["working_memory"].design_notes["preferred_standards"] == ["ISO"]
