...
def run_critical_review_specialist(
    ...
    if governance.release_status != "inquiry_ready":
        ...
        _append_unique(
            ...
            "Release status must reach inquiry_ready before RFQ handover.",
        )
    if governance.rfq_admissibility != "ready":
        _append_unique(blocking_findings, "rfq_admissibility_not_ready")
        _append_unique(
            required_corrections, "RFQ admissibility must be ready before RFQ handover."
        )
    ...
    if governance.conflicts:
        ...
        _append_unique(
            required_corrections,
            "Resolve conflicting governed inputs before RFQ handover.",
        )

    if governance.review_required:
        _append_unique(blocking_findings, "review_pending")
        _append_unique(
            required_corrections, "Resolve the pending review before RFQ handover."
        )
    ...
    if not requirement_class_id:
        ...
        _append_unique(
            required_corrections, "Resolve the requirement class before RFQ handover."
        )
    ...
    if matching_status and matching_status != "matched_primary_candidate":
        ...
        _append_unique(
            required_corrections, "Resolve deterministic matching before RFQ handover."
        )
    ...
    if (
        rfq_object
        and rfq_object.get("requirement_class") is None
        and requirement_class_id
    ):
