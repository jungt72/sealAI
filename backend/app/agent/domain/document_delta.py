...
def document_delta_from_text(
    *,
    ...
    haystack = _clean_text(
        " ".join([filename or "", category or "", " ".join(tags or []), text or ""])
    )
    fields: list[ProposedCaseDeltaField] = []
    seen: set[str] = set()
    ...
    pressure = _first_match(
        rf"(?:betriebsdruck|druck|pressure|\bp\b)\D{{0,24}}{_NUMBER}\s*bar\b", haystack
    )
    if pressure is not None:
        _append_field(
            ...
            value=_as_float(pressure),
            unit="bar",
        )

    temp_max = _first_match(
        rf"(?:temperatur\s*max(?:imum)?|temp(?:erature)?\s*max(?:imum)?|tmax)\D{{0,24}}{_NUMBER}\s*(?:deg\s*c|degc|c|°c)\b",
        haystack,
    )
    if temp_max is not None:
        _append_field(
            ...
            value=_as_float(temp_max),
            unit="degC",
        )

    temp_min = _first_match(
        rf"(?:temperatur\s*min(?:imum)?|temp(?:erature)?\s*min(?:imum)?|tmin)\D{{0,24}}{_NUMBER}\s*(?:deg\s*c|degc|c|°c)\b",
        haystack,
    )
    if temp_min is not None:
        _append_field(
            ...
            value=_as_float(temp_min),
            unit="degC",
        )

    temp = _first_match(
        rf"(?:temperatur|temperature|temp\.?|\bt\b)\D{{0,24}}{_NUMBER}\s*(?:deg\s*c|degc|c|°c)\b",
        haystack,
    )
    if temp is not None:
        _append_field(
            fields, seen, field_name="temperature_c", value=_as_float(temp), unit="degC"
        )

    speed = _first_match(
        rf"(?:drehzahl|speed|rpm|\bn\b)\D{{0,24}}{_NUMBER}\s*(?:rpm|1/min|min-1|min\^-1)\b",
        haystack,
    )
    if speed is not None:
        _append_field(
            fields, seen, field_name="speed_rpm", value=_as_float(speed), unit="rpm"
        )

    shaft = _first_match(
        rf"(?:welle|shaft|wellendurchmesser|durchmesser|diameter|\bd\b)\D{{0,24}}{_NUMBER}\s*mm\b",
        haystack,
    )
    ...
    if re.search(
        r"\b(?:rwdr|radialwellendichtring|radial shaft seal)\b",
        haystack,
        flags=re.IGNORECASE,
    ):
        _append_field(
        ...
        r"\b(?:gleitringdichtung|mechanical seal)\b", haystack, flags=re.IGNORECASE
    ):
        _append_field(
            fields,
            seen,
