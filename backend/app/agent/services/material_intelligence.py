...
def _medium_family(
    profile: dict[str, Any], medium_classification: dict[str, Any]
) -> tuple[str, str | None]:
    label = _text(
...
def _known_operating_window(profile: dict[str, Any]) -> dict[str, Any]:
    return {
        "temperature_c": _number(
            _profile_value(profile, "temperature_c", "temperature", "temperature_max_c")
        ),
        "pressure_bar": _number(
            _profile_value(
                profile, "pressure_bar", "pressure", "operating_pressure_bar"
            )
        ),
        "speed_rpm": _number(_profile_value(profile, "speed_rpm", "rpm", "speed")),
        "shaft_diameter_mm": _number(
            _profile_value(
                profile, "shaft_diameter_mm", "shaft_diameter", "diameter_mm"
            )
        ),
        "seal_type": _text(
            _profile_value(profile, "seal_type", "sealing_type", "dichtungstyp")
        ),
        "motion_type": _text(
            _profile_value(profile, "motion_type", "motion", "movement")
        ),
    }
...
def _status_for_material(
    *,
    ...
    if family not in supported:
        ...
        if material_key == "nbr" and family in {
            "wasser",
            "waessrig_salzhaltig",
            "dampf",
        }:
...
def _missing_for_material(
    profile: dict[str, Any], medium_label: str | None
) -> list[str]:
...
def _score_material(
    *,
    ...
    if family == "unknown":
        ...
        score += 26
        drivers.append(
            "Medienfamilie liegt im bekannten Prüfrahmen dieser Werkstofffamilie."
        )
    elif family in caution_families:
        score -= 8
        cautions.append(
            "Medienfamilie ist werkstoff- oder mischungsabhängig kritisch zu klären."
        )
    elif status == "excluded_by_known_constraint":
        score -= 36
        cautions.append(
            "Eine bekannte Angabe spricht deutlich gegen diese Werkstofffamilie."
        )
    else:
        score -= 16
        cautions.append(
            "Aus den bekannten Angaben entsteht aktuell kein vorrangiges Prüfargument."
        )

    if known_key == material_key:
        score += 8
        drivers.append(
            "Der Werkstoff wurde im Fall bereits genannt und bleibt deshalb im Vergleich sichtbar."
        )
    ...
    if isinstance(temperature_window, tuple) and len(temperature_window) == 2:
        ...
        if temperature_c is None:
            cautions.append(
                "Temperatur fehlt; dadurch bleibt der Werkstoffvergleich offen."
            )
        elif low <= temperature_c <= high:
            score += 12
            drivers.append(
                "Die Temperatur liegt im groben Orientierungsfenster dieser Werkstofffamilie."
            )
            if temperature_c > high * 0.85:
                score -= 4
                cautions.append(
                    "Die Temperatur liegt nahe am oberen Orientierungsbereich; Compounddaten werden wichtig."
                )
        else:
            score -= 28
            cautions.append(
                "Die Temperatur liegt außerhalb des groben Orientierungsfensters dieser Werkstofffamilie."
            )
    ...
    is_rotary = (
        "rot" in motion_text
        or "welle" in seal_text
        or "rwdr" in seal_text
        or "radial" in seal_text
    )
    is_linear = (
        "linear" in motion_text
        or "hydraul" in seal_text
        or "kolben" in seal_text
        or "stange" in seal_text
    )
    is_static = (
        "statisch" in motion_text or "flach" in seal_text or "flansch" in seal_text
    )
    ...
    if is_rotary:
        if material_key in {"nbr", "fkm", "hnbr"}:
            score += 8
            drivers.append(
                "Rotierende Wellendichtungen nutzen häufig elastomere Werkstofffenster."
            )
        if material_key == "ptfe":
            score += 3
            cautions.append(
                "PTFE kann bei rotierenden Dichtstellen konstruktiv interessant sein, braucht aber Gegenlauf, Vorspannung und Bauformdaten."
            )
        if material_key == "pu":
            score -= 12
            cautions.append(
                "PU ist bei rotierenden Dichtstellen ohne Bauformkontext nicht vorrangig."
            )
    elif is_linear:
        if material_key in {"pu", "nbr", "hnbr"}:
            score += 10
            drivers.append(
                "Lineare Hydraulik-/Pneumatikdichtungen können mechanische Werkstofffenster stärker gewichten."
            )
    elif is_static:
        if material_key in {"epdm", "ptfe", "ffkm", "vmq"}:
            score += 6
            drivers.append(
                "Bei statischen Dichtstellen können Medien- und Nachweisfenster stärker als Reibung dominieren."
            )
    ...
    if family == "dampf":
        if material_key == "epdm":
            score += 16
            drivers.append(
                "Dampf/Heißwasser rückt EPDM häufig früh in den Prüfrahmen, sofern Öl ausgeschlossen ist."
            )
        elif material_key == "fkm":
            score -= 12
            cautions.append(
                "FKM ist bei Dampf/Heißwasser stark compound- und datenblattabhängig."
            )
        elif material_key == "ptfe":
            score -= 8
            cautions.append(
                "PTFE ist chemisch breit, aber Dampfzyklen, Kriechen, Vorspannung und Bauform können limitieren."
            )
        elif material_key == "nbr":
            score -= 30
            cautions.append(
                "NBR ist für Dampf/Heißwasser aus den bekannten Angaben niedrig zu priorisieren."
            )
    ...
    if family == "waessrig_salzhaltig":
        if material_key in {"epdm", "ptfe"}:
            score += 10
            drivers.append(
                "Salzhaltige wässrige Medien verlangen frühe Prüfung von Medienkontakt und Korrosionsumfeld."
            )
        if material_key == "fkm":
            score -= 8
            cautions.append(
                "FKM ist bei wässrig-salzhaltigem Kontakt nicht automatisch vorrangig."
            )
        if material_key == "nbr":
            score -= 18
            cautions.append(
                "NBR bleibt bei Salzwasser ohne weitere Begründung niedrig priorisiert."
            )
    ...
    if family in {"oel", "hydraulikoel"}:
        if material_key in {"nbr", "hnbr", "fkm", "pu"}:
            score += 12
            drivers.append(
                "Öl- oder Hydraulikölkontakt stützt dieses Werkstofffenster gegenüber wasserorientierten Familien."
            )
        if material_key == "epdm":
            score -= 34
            cautions.append(
                "Öl- oder Hydraulikölkontakt spricht deutlich gegen EPDM als frühe Richtung."
            )

    if pressure_bar is None:
        cautions.append(
            "Druck oder Druckdifferenz fehlen; Auspressung, Bauformgrenzen und Spaltmaß bleiben offen."
        )
    elif pressure_bar > 20:
        cautions.append(
            "Höherer Druck verschiebt die Bewertung stark in Richtung Bauform, Stützring und Spaltmaß."
        )
        if material_key == "pu":
            score += 5
    else:
        score += 3
        drivers.append(
            "Der bekannte Druck erzeugt aktuell keinen Hochdruck-Warnpunkt im Werkstofffenster."
        )

    if status == "requires_supplier_review":
        score = min(score, 74)
        cautions.append(
            "Lieferanten- oder Herstellerdaten bleiben für diese Werkstofffamilie früh erforderlich."
        )
    ...
    *,
    ...
    if status == "candidate_to_check":
        relevant_missing = [
            ...
            if item
            in {
                "Dichtprinzip oder Dichtungstyp",
                "Druck oder Druckdifferenz",
                "Temperatur",
            }
        ]
    ...
    if family == "waessrig_salzhaltig" and material_key in {"epdm", "ptfe"}:
        why.insert(
            0,
            "Salzhaltiges Wasser verlangt fruehe Trennung zwischen Dichtwerkstoff und metallischen Kontaktteilen.",
        )
    if family in {"oel", "hydraulikoel"} and material_key in {
        "nbr",
        "hnbr",
        "fkm",
        "pu",
        ...
        why.insert(
            0,
            "Oelkontakt grenzt das elastomere Prueffenster deutlich gegen wassernahe Familien ab.",
        )
    ...
    return {
        ...
        "confidence": "medium"
        if status in {"candidate_to_check", "needs_more_data"}
        else "low",
...
def _alternatives(candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ranked = [
        ...
        if item["status"]
        in {"candidate_to_check", "needs_more_data", "requires_supplier_review"}
    ]
        ...
        alternatives.append(
            {
                ...
                "missing_for_decision": list(
                    dict.fromkeys(
                        left["blocking_unknowns"] + right["blocking_unknowns"]
                    )
                )[:5],
            }
        )
...
def build_material_intelligence_projection(
    *,
    ...
    window = _known_operating_window(profile)
    known_material = (
        _text(_profile_value(profile, "material", "material_family", "werkstoff"))
        or None
    )
    ...
    if medium_family == "waessrig_salzhaltig":
        rfq_notes.insert(
            0,
            "Bei Salzwasser muessen neben dem Dichtwerkstoff auch Welle, Gehaeuse, Feder und Stuetzteile betrachtet werden.",
        )
    if any(item["material_key"] in {"fkm", "ptfe", "ffkm"} for item in candidates):
        rfq_notes.append(
            "Bei fluorierten Werkstoffen koennen PFAS-, Liefer- und Dokumentationsfragen relevant werden."
        )
