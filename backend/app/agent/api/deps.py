...
def _canonical_scope(
    current_user: RequestUser, *, case_id: str
) -> tuple[str, str, str]:
...
def _resolve_payload_binding_level(
    default_binding_level: str, *, case_state: Optional[Dict[str, Any]]
) -> str:
    if not case_state:
...
def _runtime_mode_for_pre_gate(
    classification: str,
) -> Literal["CONVERSATION", "EXPLORATION", "GOVERNED"]:
