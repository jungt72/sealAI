...
def _determine_response_class(state: GraphState) -> str:
    ...
    result = _OUTPUT_CLASSIFIER.classify(
        OutputClassificationInput(
            ...
            has_compute_results=bool(state.compute_results),
            recommendation_ready=_is_recommendation_ready(state),
            has_blocking_evidence_gaps=bool(
                _blocking_evidence_gaps_for_preselection(state)
            ),
        )
    )
    return result.output_class.value
