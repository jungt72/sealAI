...
_MOTION_PATTERNS: tuple[tuple[re.Pattern[str], str, str], ...] = (
    ...
    (
        re.compile(
            r"\b(?:schiffsschraube|rotierende?\s+welle|rotierend\w*|rwdr|radialwellendichtring|shaft)\b",
            re.IGNORECASE,
        ),
        "rotary",
        "high",
    ...
    (
        re.compile(
            r"\b(?:statische?\s+abdichtung|statisch\w*|gehaeuse|gehäuse)\b",
            re.IGNORECASE,
        ),
        "static",
        "high",
...
_APPLICATION_PATTERNS: tuple[tuple[re.Pattern[str], str, str], ...] = (
    (
        re.compile(
            r"\b(?:lineare?\s+beweg\w*|linear\w*|kolbenstange|hubstange|zylinder)\b",
            re.IGNORECASE,
        ),
        "linear_sealing",
        "high",
    ...
    (
        re.compile(r"\b(?:schiffsschraube)\b", re.IGNORECASE),
        "marine_propulsion",
        "high",
    ),
    (
        re.compile(
            r"\b(?:draussen\s+halten|draußen\s+halten|aussen\s+abdichten|außen\s+abdichten)\b",
            re.IGNORECASE,
        ),
        "external_sealing",
        "high",
    ...
    (
        re.compile(
            r"\b(?:rotierende?\s+welle|welle|shaft|rwdr|radialwellendichtring)\b",
            re.IGNORECASE,
        ),
        "shaft_sealing",
        "medium",
    ...
    (
        re.compile(r"\b(?:statische?\s+abdichtung|statisch\w*)\b", re.IGNORECASE),
        "static_sealing",
        "medium",
    ),
)
...
def _derive_hint(
    *,
    ...
    existing = (
        previous if isinstance(previous, ContextHintState) else ContextHintState()
    )
    ...
    for pattern, label, confidence in patterns:
        if not pattern.search(text):
            continue
        source_turn_index = (
            max(observed.source_turns)
            if observed.source_turns
            else existing.source_turn_index
        )
        source_turn_ref = (
            f"turn:{source_turn_index}"
            if source_turn_index is not None
            else existing.source_turn_ref
        )
