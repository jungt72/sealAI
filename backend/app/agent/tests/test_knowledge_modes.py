...
def test_knowledge_case_mutating_when_new_facts_present() -> None:
    mode = resolve_knowledge_mode("Wir verwenden FKM, Öltemperatur 100 °C", has_active_case=True)
...
def test_comparison_modes_split_by_case_presence() -> None:
    assert resolve_knowledge_mode("Was ist besser, NBR oder FKM?", has_active_case=False) == "comparison_general"
    assert resolve_knowledge_mode("NBR oder FKM für meinen Fall?", has_active_case=True) == "comparison_case_aware"


def test_norm_documentation_mode() -> None:
    assert (
        resolve_knowledge_mode("Was bedeutet WRAS bei einem RWDR?", has_active_case=True)
...
def test_why_question_mode_requires_active_case() -> None:
    ...
    assert resolve_knowledge_mode("Warum ist das wichtig?", has_active_case=False) == "knowledge_general"
...
def test_non_mutating_modes_leave_case_revision_unchanged(message: str, has_case: bool) -> None:
...
def test_case_mutating_runs_only_supplied_facts_through_state_gate() -> None:
    ...
    assert applied.issubset({"temperature_c", "pressure_bar", "shaft_diameter_mm", "speed_rpm", "medium", "material"})


# --- why-question reply (§8.10) reuses senior_engineer_short + No-Go guard ---


def test_why_question_reply_uses_senior_engineer_short_and_suppresses_disclaimer() -> None:
