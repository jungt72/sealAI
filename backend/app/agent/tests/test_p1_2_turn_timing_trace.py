...
def test_timer_none_when_not_started():
    ...
    assert (fp is None and lat is None) or (
        isinstance(fp, (int, type(None))) and isinstance(lat, int)
    )
...
def test_timer_marks_progress_and_latency():
    ...
    assert (
        isinstance(latency_ms, int) and latency_ms >= first_progress_ms
    )  # total >= first chunk
...
def test_final_state_update_carries_timing_for_any_route():
    ...
    trace = event["trace"]
    assert (
        isinstance(trace["first_progress_ms"], int) and trace["first_progress_ms"] >= 0
    )
...
def test_mobile_nested_trace_is_left_unchanged():
    ...
    payload = {
        ...
        "assistant_turn_envelope": {
            "trace": {"route": "mobile_leakage_triage", "first_progress_ms": 0}
        },
    }
    event = builder.event(payload, event_type="state_update", is_final=True)
    assert (
        event["assistant_turn_envelope"]["trace"]["first_progress_ms"] == 0
    )  # byte-identical
...
def test_frame_serializes_with_timing():
    ...
    frame = builder.frame(
        {"type": "state_update"}, event_type="state_update", is_final=True
    )
