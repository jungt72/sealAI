...
def test_build_final_reply_prefers_canonical_dispatch_facing_truth_over_stale_selection_fields():
    ...
    assert build_final_reply(selection_state, case_state=case_state).startswith(
        NEUTRAL_SCOPE_REPLY
    )
...
def test_reply_constants_are_german():
    for name, value in [
        ...
        assert (
            "governed recommendation" not in value
...
_KEY_STATUS_PATTERN = re.compile(
    r"\b\w+:\w+"
)  # matches "pressure:plausibility_failure"
    ...
    note = _build_integrity_note(
        {
            ...
            "blocking_keys": ["pressure", "temperature"],
        },
        {
            "statuses": {
                "pressure": "plausibility_failure",
                "temperature": "unit_ambiguous",
            }
        },
    )
    assert not _KEY_STATUS_PATTERN.search(
        note
