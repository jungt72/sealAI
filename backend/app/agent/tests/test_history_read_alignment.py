...
@pytest.mark.asyncio
async def test_chat_history_prefers_live_governed_history(
    ...
    load_snapshot = AsyncMock(
        side_effect=AssertionError(
            "snapshot should not be read when live governed exists"
        )
    )
    load_structured = AsyncMock(
        side_effect=AssertionError(
            "structured fallback should not be read when governed exists"
        )
    )

    monkeypatch.setattr("app.agent.api.loaders._load_live_governed_state", load_live)
    monkeypatch.setattr(
        history_routes, "get_latest_governed_case_snapshot_async", load_snapshot
    )
...
async def test_chat_history_uses_latest_snapshot_before_structured_fallback(
    ...
    monkeypatch.setattr(
        "app.agent.api.loaders._load_live_governed_state", AsyncMock(return_value=None)
    )
    monkeypatch.setattr(
        history_routes, "get_latest_governed_case_snapshot_async", _load_snapshot
    )
    monkeypatch.setattr(
        history_routes,
        ...
        AsyncMock(
            side_effect=AssertionError(
                "structured fallback should not be read when snapshot exists"
            )
        ),
    )
...
async def test_chat_history_returns_empty_when_all_sources_absent(
    ...
    monkeypatch.setattr(
        history_routes, "load_structured_case", AsyncMock(return_value=None)
    )
