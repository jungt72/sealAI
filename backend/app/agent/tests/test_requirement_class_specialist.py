...
def test_clear_ptfe_steam_case_returns_preferred_requirement_class() -> None:
    ...
    assert [
        candidate.class_id for candidate in result.requirement_class_candidates
    ] == ["PTFE10"]
    ...
    assert any(
        "asserted PTFE material family" in item for item in result.scope_of_validity
    )
...
def test_geometry_install_hints_shape_scope_without_changing_authority() -> None:
    ...
    assert any(
        "rotary installation context" in item for item in result.scope_of_validity
    )
...
def test_open_case_returns_open_points_without_artificial_certainty() -> None:
    ...
    assert result.scope_of_validity == (
...
def test_steam_without_material_stays_bounded_and_requests_follow_up_material() -> None:
    ...
    assert result.open_points == ("material",)
    assert any(
        "without a confirmed material family" in item
        for item in result.scope_of_validity
    )
