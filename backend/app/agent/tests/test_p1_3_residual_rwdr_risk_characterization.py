...
def test_runout_risk_rwdr_above_threshold():
    r = _risk(
        evaluate_risks({"runout_mm": 0.3}, engineering_path="rwdr"), "runout_risk"
    )
...
def test_runout_risk_rwdr_below_threshold():
    r = _risk(
        evaluate_risks({"runout_mm": 0.1}, engineering_path="rwdr"), "runout_risk"
    )
...
def test_runout_risk_absent_for_non_rwdr():
    assert (
        _risk(
            evaluate_risks({"runout_mm": 0.3}, engineering_path="o_ring"), "runout_risk"
        )
        is None
    )
    assert (
        _risk(
            evaluate_risks({"runout_mm": 0.3}, engineering_path="ms_pump"),
            "runout_risk",
        )
        is None
    )
