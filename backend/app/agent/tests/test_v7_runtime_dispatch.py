...
from app.agent.communication.conversation_controller_v7 import ConversationControllerInput, ConversationControllerV7
...
def _active_state() -> GovernedSessionState:
    return GovernedSessionState(
        conversation_messages=[
            ConversationMessage(role="user", content="servus, ich brauche eine Dichtungsloesung"),
            ConversationMessage(role="assistant", content="Welches Medium soll abgedichtet werden?"),
        ],
...
@pytest.mark.asyncio
async def test_active_case_material_followup_routes_as_v7_side_question(monkeypatch: pytest.MonkeyPatch) -> None:
    state = _active_state()
...
@pytest.mark.asyncio
async def test_no_case_material_comparison_stays_direct_knowledge_without_case_mutation(monkeypatch: pytest.MonkeyPatch) -> None:
    load_state = AsyncMock(side_effect=AssertionError("no-case knowledge without session must not load governed state"))
    monkeypatch.setattr("app.agent.api.dispatch._load_live_governed_state", load_state)
...
@pytest.mark.asyncio
async def test_chat_endpoint_preserves_v7_side_question_as_knowledge_answer(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("app.agent.api.routes.chat._resolve_runtime_dispatch", AsyncMock(return_value=dispatch))
    build_side = AsyncMock(return_value=side_answer)
    monkeypatch.setattr("app.agent.api.routes.chat.build_case_side_knowledge_response", build_side)
