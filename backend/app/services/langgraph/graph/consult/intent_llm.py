...
def intent_router_node(state: ConsultState) -> ConsultState:
    ...
    new_state: ConsultState = dict(state)
    new_state["route"] = route
    # Optional: Phase für Logging/Tracing
    try:
        new_state["phase"] = "intent_router"  # type: ignore[typeddict-item]
    except Exception:
        pass
    return new_state

__all__ = ["intent_router_node", "ALLOWED_ROUTES"]
