            ...
            LLMOperatingPoint(
                conditions=[
                    LLMCondition(
                        parameter="medium",
                        raw_value="Wasser",
                        inferred_operator=Operator.EQ,
                        evidence_ref="S. 1",
                    )
                ],
                limits=[
                    LLMLimit(
                        limit_type="pressure_max_bar",
                        raw_value="5 - 10 bar",
                        evidence_ref="S. 2",
                    ),
                    LLMLimit(
                        limit_type="temperature_max_c",
                        raw_value="120,5",
                        evidence_ref="S. 2",
                    ),
                ],
...
def test_bp30_safety_exclusion_does_not_block_validated():
    ...
    llm_output = LLMDocumentExtraction(
        ...
        safety_exclusions=[
            "Nicht für aggressive Säuren geeignet.",
            "Kein Einsatz bei Dampf > 150 °C.",
        ],
    )
    ...
    ), "BP30 muss mindestens einen validen Qdrant-Point liefern."

    # Safety-Warnings müssen im quarantine_report als Info erscheinen
    safety_msgs = [
        m for m in result.quarantine_report if "SAFETY_EXCLUSION_DETECTED" in m
    ]
    assert (
        safety_msgs
    ), "Safety-Exclusion-Warnung muss im Report enthalten sein (informativ)."
...
def test_bp30_material_family_in_vector_text():
    ...
    result = process_document_pipeline(
        llm_output, "logical_bp30_key", additional_metadata
    )
    ...
    assert (
        "FKM" in vector_text
    ), f"material_family 'FKM' fehlt im vector_text:\n{vector_text}"
    assert (
        "Fluorocarbon" in vector_text
    ), f"polymer_name 'Fluorocarbon' fehlt im vector_text:\n{vector_text}"

    print(
        f"=== BP30: Fix 1 OK — vector_text enthält material_family + polymer_name ==="
    )
    print(f"Vector Text:\n{vector_text}")
    ...
    llm_output = LLMDocumentExtraction(
        ...
        operating_points=[
            LLMOperatingPoint(
                conditions=[
                    LLMCondition(
                        parameter="speed_max_m_s",
                        raw_value="0.5",
                        inferred_operator=Operator.LTE,
                        evidence_ref="S. 3",
                    ),
                ],
                limits=[
