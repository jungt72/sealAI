...
EMBEDDINGS_CACHE_FOLDER = os.getenv(
    "EMBEDDINGS_CACHE_FOLDER", "/app/data/models"
).strip()
EMBEDDINGS_CACHE_FALLBACK = os.getenv(
    "EMBEDDINGS_CACHE_FALLBACK", "/tmp/sealai-models"
).strip()
...
def _build_sources(hits: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for hit in hits:
        ...
        doc_id = (
            metadata.get("document_id") or metadata.get("doc_id") or metadata.get("id")
        )
...
# ─────────────────────────────────────────────────────────────────────────────
def init_bm25(
    redis_url: Optional[str] = None, index_name: Optional[str] = None
) -> Optional[Any]:
...
def prewarm_embeddings() -> None:
    ...
    try:
        ...
        _event(
            "embeddings_failed", model=EMB_MODEL_NAME, error=f"{type(e).__name__}: {e}"
        )
...
def _hit_key(hit: Dict[str, Any]) -> str:
    metadata = hit.get("metadata") or {}
    chunk_id = (
        metadata.get("chunk_id") or metadata.get("point_id") or metadata.get("id")
    )
...
def _rrf_fuse(
    vector_hits: List[Dict[str, Any]], bm25_hits: List[Dict[str, Any]], k0: int = RRF_K
) -> List[Dict[str, Any]]:
...
def _bm25_search(
    ...
    try:
        ...
        return [], _qdrant_error(
            "bm25_unavailable", None, f"{type(exc).__name__}: {exc}"
        )
    try:
        return bm25_repo.search(
            collection, query, top_k=top_k, metadata_filters=metadata_filters
        ), None
    except Exception as exc:
...
def _normalize_supported_metadata_filters(
...
def _hit_matches_metadata_filters(
    metadata: Dict[str, Any], metadata_filters: Optional[Dict[str, Any]]
) -> bool:
...
def _build_qdrant_filter(
    ...
    visibility_user_id: Optional[str] = (
        metadata_filters.get("_visibility_user_id") or None
    )
    ...
    if len(deduped) == 1:
        tenant_condition: Dict[str, Any] = {
            "key": "tenant_id",
            "match": {"value": deduped[0]},
        }
    else:
    ...
    for raw_key, raw_value in metadata_filters.items():
        ...
        if key in _TAGS_FILTER_KEYS:
            ...
            if tag_values:
                result["must"].append(
                    {"key": "metadata.tags", "match": {"any": tag_values}}
                )
            continue
...
def _qdrant_search_with_retry(
    ...
    effective_timeout_s = (
        float(timeout_s) if timeout_s is not None else QDRANT_TIMEOUT_S
    )
    ...
    for attempt in range(QDRANT_RETRY_ATTEMPTS):
        ...
        try:
            ...
            if sparse_vector is not None:
                ...
                prefetch = [
                    models.Prefetch(
                        query=query_vec, using=QDRANT_VECTOR_NAME, limit=top_k
                    ),
                    models.Prefetch(
                        query=sparse_vector,
            ...
            top_score = (
                hits[0].score
                if hits and hasattr(hits[0], "score")
                else (
                    hits[0].get("vector_score")
            ...
            if (
                ...
                and not _should_retry_status(status)
            ):
                break
    ...
    if last_error:
        _event(
            ...
            reason=last_error.get("message", "error"),
            collection=collection,
        )
    return [], {
        "attempts": attempts,
...
def _rerank_if_enabled(
    query: str, hits: List[Dict[str, Any]], use_rerank: bool
) -> List[Dict[str, Any]]:
...
def hybrid_retrieve(
    *,
    ...
    effective_user_id = user_id or (
        tenant if tenant and tenant != RAG_SHARED_TENANT_ID else None
    )
    ...
    log.debug(
        "rag.qdrant_filter",
        extra={
            "filter": str(qdrant_filter)[:300] if qdrant_filter else None,
            "tenant": tenant,
        },
    )
    ...
    if isinstance(bm25_tenant_value, (list, tuple, set)):
        ...
        preferred_tenant = next(
            (v for v in bm25_tenant_candidates if v != RAG_SHARED_TENANT_ID), None
        )
        if preferred_tenant:
    ...
    if USE_BM25:
        ...
        if bm25_hits:
            bm25_hits = [
                ...
                if _hit_matches_metadata_filters(
                    dict(hit.get("metadata") or {}), filters
                )
            ]
    ...
    metrics: Dict[str, Any] = {
        ...
        "top_scores": [_score_value(hit) for hit in merged][:5],
    ...
    if (
        merged
        and float(merged[0].get("fused_score") or merged[0].get("vector_score") or 0.0)
        < 0.1
    ):
        log.warning(
            ...
            extra={
                "query": q[:100],
                "top_score": merged[0].get("fused_score")
                or merged[0].get("vector_score"),
                "tenant": tenant,
            },
        )
...
def _fallback_external_search(
    query: str, *, tenant: Optional[str] = None, limit: int = FINAL_K
) -> List[Dict[str, Any]]:
    """Compatibility stub.
