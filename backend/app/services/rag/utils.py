...
UPLOAD_ROOT = (
    os.getenv("RAG_UPLOAD_DIR") or "/app/data/uploads"
).strip() or "/app/data/uploads"
...
def validate_upload_signature(
    *, extension: str, content_type: str | None, sample: bytes
) -> str:
    ext = extension.lower()
    ...
    if normalized_content_type and normalized_content_type not in allowed_for_ext:
        raise HTTPException(
            status_code=415,
            detail=error_detail(
                "unsupported_content_type", content_type=normalized_content_type
            ),
        )

    if ext == ".pdf" and not sample.startswith(b"%PDF-"):
        raise HTTPException(
            status_code=415, detail=error_detail("upload_signature_mismatch")
        )
