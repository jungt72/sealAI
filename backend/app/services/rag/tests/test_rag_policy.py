...
def test_build_qdrant_filter_tenant_only():
    ...
    assert q_filter == {"must": [{"key": "tenant_id", "match": {"value": "tenant-1"}}]}
...
def test_hybrid_retrieve_filter_logic_unit(tenant, expected_tenants, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.rag.rag_orchestrator._build_qdrant_filter", mock_build_filter
    )
    monkeypatch.setattr(
