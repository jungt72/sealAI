...
def test_rag_ingest_uses_prefixed_collection_with_tenant(monkeypatch, tmp_path) -> None:
    rag_ingest = _import_rag_ingest()
    captured = {}

    def fake_from_documents(*_args, **kwargs):
    ...
    monkeypatch.setattr(rag_ingest, "load_document", lambda _path: [_DummyDoc("hello")])
...
def test_rag_ingest_uses_base_collection_without_tenant(monkeypatch, tmp_path) -> None:
    ...
    rag_ingest.ingest_file(str(file_path), tenant_id=None)
