...
def transition_to_published_bulletproof(
    ...
    if (
        not target
        or target[0].payload is None
        or target[0].payload.get("document_meta", {}).get("status") != "VALIDATED"
    ):
        raise PublishTransitionError(
            f"Point {new_qdrant_id} nicht gefunden oder nicht VALIDATED."
        )
    ...
    old_points_scroll, _ = client.scroll(
        ...
        scroll_filter=Filter(
            must=[
                FieldCondition(
                    key="document_meta.logical_document_key",
                    match=MatchValue(value=logical_document_key),
                )
            ],
            must_not=[
                FieldCondition(
                    key="document_meta.version_id",
                    match=MatchValue(value=new_version_id),
                )
            ],
        ),
        limit=1000,
    ...
    client.batch_update_points(
        collection_name=collection_name, update_operations=operations, wait=True
    )

    # 4. Final Verify
    verify_new = client.retrieve(
        collection_name=collection_name,
        ids=[new_qdrant_id],
        with_payload=["document_meta.status"],
    )
    if (
        not verify_new
        or verify_new[0].payload.get("document_meta", {}).get("status") != "PUBLISHED"
    ):
