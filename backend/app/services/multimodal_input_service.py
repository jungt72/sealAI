...
class MultimodalInputService:
    def process_article_number(self, raw: str) -> IntakeExtraction:
        ...
        if dims is not None:
            params.update(
                {
                    "shaft.diameter_mm": dims[0],
                    "housing.bore_diameter_mm": dims[1],
                    "seal.width_mm": dims[2],
                }
            )
            confidence.update(
                {
                    "shaft.diameter_mm": 0.8,
                    "housing.bore_diameter_mm": 0.8,
                    "seal.width_mm": 0.8,
                }
            )
        upper = raw.upper()
        ...
        return IntakeExtraction(
            ...
            "article-number-v1",
            True,
            ("Please confirm decoded dimensions and seal type.",),
        )
    ...
    def process_free_text(self, raw: str) -> IntakeExtraction:
        ...
        if dims is not None:
            ...
            confidence.update(
                {
                    "shaft.diameter_mm": 0.7,
                    "housing.bore_diameter_mm": 0.7,
                    "seal.width_mm": 0.7,
                }
            )
        rpm = re.search(r"(\d+(?:[.,]\d+)?)\s*(?:rpm|u\.?/?min)", raw, re.I)
        if rpm:
            params["operating.shaft_speed.rpm_nom"] = float(
                rpm.group(1).replace(",", ".")
            )
    ...
    def process_photo(self, raw_reference: str) -> IntakeExtraction:
        return IntakeExtraction(
            ...
            {},
            "user_photo",
            "photo-placeholder-v1",
            True,
            (
                "Please confirm seal type and dimensions; exact compound and dimensions cannot be inferred without evidence.",
            ),
            "MVP stores photo evidence but does not claim exact visual engineering truth.",
        )

    def process_datasheet_fragment(self, raw_reference: str) -> IntakeExtraction:
        return IntakeExtraction(
            InputType.DATASHEET_FRAGMENT,
            raw_reference,
            {},
            {},
            "documented",
            "datasheet-placeholder-v1",
            True,
            ("Please confirm visible table values from the datasheet.",),
        )
