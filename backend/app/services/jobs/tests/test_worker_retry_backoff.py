...
@pytest.mark.anyio
async def test_worker_retry_backoff(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(worker, "MAX_ATTEMPTS", 2)
    monkeypatch.setattr(worker, "BACKOFF_BASE_SEC", 1)
    monkeypatch.setattr(worker, "BACKOFF_MAX_SEC", 10)
    ...
    def failing_ingest(_path, **_kwargs):
        raise RuntimeError("boom")

    _ = await worker.consume_redis_job_once(
        redis,
    ...
    assert worker.SCHEDULED_QUEUE in redis.zsets

    doc.attempt_count = worker.MAX_ATTEMPTS
    await redis.rpush(worker.JOB_QUEUE, json.dumps(payload))
    _ = await worker.consume_redis_job_once(
        redis,
        ingest_func=failing_ingest,
    ...
    assert doc.attempt_count == worker.MAX_ATTEMPTS + 1
