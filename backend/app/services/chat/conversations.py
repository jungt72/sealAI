...
Legacy keys (pre-A5, read/delete fallback only — never written to):
...
# ---------------------------------------------------------------------------
# A5 tenant-scoped key builders (primary path)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Legacy key builders (pre-A5, read/delete fallback only — never written to)
# ---------------------------------------------------------------------------
...
def upsert_conversation(
    ...
    *,
    ...
    A5: when tenant_id is provided, writes to the tenant-scoped key pair
    (`chat:conversations:{tenant_id}:{owner_id}` / `chat:conversation:{tenant_id}:{owner_id}:…`).
    When tenant_id is absent (legacy callers), falls back to the owner-only key pair.
    """
    ...
    try:
        r = _redis_client()
        # A5: use tenant-scoped keys when tenant_id is available; legacy otherwise.
        if tenant_id:
...
def _collect_from_sorted_set(
    ...
    Stale sorted-set members (expired hash) are removed lazily.
    """
...
def _collect_for_owner(owner_id: str, *, tenant_id: str | None = None) -> List[ConversationMeta]:
    """Collect all conversations for this owner.

    A5: when tenant_id is provided, reads from the tenant-scoped sorted set first.
    Always also reads from the legacy owner-only sorted set as a fallback for pre-A5
    records.  New-key entries take unconditional priority over legacy entries for the
    ...
    try:
        ...
        # A5 primary: tenant-scoped sorted set
        if tenant_id:
...
def list_conversations(
    ...
    *,
    ...
    A5: pass tenant_id so the internal collection uses tenant-scoped keys.
    """
    merged: Dict[str, ConversationMeta] = {}
...
def delete_conversation(
    ...
    *,
    ...
    """Remove the hash + sorted-set member for a conversation.

    A5: deletes from both the tenant-scoped key pair (if tenant_id provided) and the
    legacy owner-only key pair.  Redis DEL / ZREM on non-existent keys are no-ops,
    so this is safe to call even if the record only exists under one of the key styles.
    """
    if not owner_id or not conversation_id:
        return
    try:
