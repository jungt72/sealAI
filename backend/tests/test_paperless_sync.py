...
async def test_paperless_sync_removes_existing_document_when_rag_flag_is_removed(
                ...
                "results": [
                    {
                        ...
                        "original_file_name": "spec.pdf",
                        "modified": "2026-03-12T10:00:00Z",
                        "tags": ["doc_type:datasheet", "sts_mat:PTFE"],
                    }
                ]
            },
        ),
    ...
    assert result["tag_not_enabled"] == 1
    stored = session.docs["doc-removed"]
    assert stored.enabled is False
    assert stored.status == "removed"
    assert stored.extraction_status == "removed"
    assert stored.error == "paperless_rag_flag_removed"
...
async def test_paperless_sync_removes_existing_document_when_source_disappears(
    ...
    assert result["removed"] == 1
    stored = session.docs["doc-missing"]
    assert stored.enabled is False
    assert stored.status == "removed"
