...
def test_registry_authority_default_is_demo_only():
    assert (
        PromotedCandidateRegistryRecordDTO(
            registry_record_id="r1", material_family="PTFE"
        ).registry_authority
        == "demo_only"
    )
...
def test_unknown_registry_authority_is_rejected():
    with pytest.raises(ValueError):
        PromotedCandidateRegistryRecordDTO(
            registry_record_id="r1", material_family="PTFE", registry_authority="bad"
        )


def test_registry_json_entry_has_demo_only_authority():
    assert (
        load_promoted_candidate_registry_records()[0].registry_authority == "demo_only"
    )
...
def test_demo_only_candidate_gets_transition_source_origin():
    assessment = classify_material_candidate_sources(
        candidates=_CANDIDATE, relevant_fact_cards=[]
    )[0]
...
def test_material_core_output_is_exploratory_with_demo_only_registry():
    core_output = evaluate_material_qualification_core(
        relevant_fact_cards=[_FACT_CARD], asserted_state={}, governance_state={}
    )
