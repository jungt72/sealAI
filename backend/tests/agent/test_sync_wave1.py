...
def test_sync_no_longer_writes_upstream():
    ...
    """
    state = {
        "working_profile": {"speed": 5000, "diameter": 30, "pressure": 10},
    ...
    assert (
        "diameter" not in updated_state["sealing_state"]["asserted"]["machine_profile"]
    )
    assert (
        "pressure" not in updated_state["sealing_state"]["asserted"]["machine_profile"]
    )
    ...
    state = {
        ...
        "sealing_state": {
            "asserted": {
                ...
                "operating_conditions": {"temperature": 120, "pressure": 15},
            }
        },
    }
...
def test_live_calc_tile_generation_persists():
    ...
    """
    state = {
        "working_profile": {"v_m_s": 7.85, "pv_value": 78.5, "speed": 5000},
