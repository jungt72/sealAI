...
def test_material_temperature_validation(fkm_profile):
    ...
    # 150°C ist OK
    assert (
        validator.validate_temperature(PhysicalParameter(value=150.0, unit="C")) is True
    )

    # 250°C ist zu heiß
    assert (
        validator.validate_temperature(PhysicalParameter(value=250.0, unit="C"))
        is False
    )

    # -30°C ist zu kalt
    assert (
        validator.validate_temperature(PhysicalParameter(value=-30.0, unit="C"))
        is False
    )
