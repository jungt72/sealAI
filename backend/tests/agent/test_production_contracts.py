...
def test_chat_stream_endpoint_contract(client, mock_user):
    ...
    with patch(
        ...
        new=AsyncMock(return_value=mock_dispatch),
    ):
        response = client.post(
            "/api/agent/chat/stream", json={"message": "hi", "session_id": "s1"}
        )
...
def test_chat_history_contract(client, mock_user):
    ...
    with patch(
        ...
        "app.agent.api.loaders._load_live_governed_state", new=AsyncMock()
    ) as mock_load:
...
def test_dispatch_logic_isolation(client, mock_user):
    ...
    with patch(
        "app.services.pre_gate_classifier.PreGateClassifier.classify"
    ) as mock_classify:
