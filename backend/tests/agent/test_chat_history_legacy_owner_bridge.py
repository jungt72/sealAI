...
def test_legacy_owner_history_loads_structured_case_via_entry_owner(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda owner_id, legacy_owner_id=None, tenant_id=None: [
            _conversation("legacy-sub")
        ],
    )

    async def _load(*, tenant_id: str, owner_id: str, case_id: str):
        captured.update(
            {"tenant_id": tenant_id, "owner_id": owner_id, "case_id": case_id}
        )
    ...
    payload = asyncio.run(
        get_conversation_history("conv-1", current_user=_request_user())
    )
...
def test_legacy_owner_rename_upserts_via_entry_owner(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.api.v1.endpoints.chat_history.upsert_conversation",
...
def test_legacy_owner_delete_deletes_structured_case_via_entry_owner(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.api.v1.endpoints.chat_history.delete_conversation",
        lambda owner_id,
        conversation_id,
        tenant_id=None,
        reason="manual": captured.update(
            {"metadata_owner_id": owner_id, "metadata_tenant_id": tenant_id}
        ),
    )

    async def _delete_structured_case(*, tenant_id: str, owner_id: str, case_id: str):
        captured.update(
            {"structured_owner_id": owner_id, "structured_tenant_id": tenant_id}
        )

    monkeypatch.setattr(
        "app.api.v1.endpoints.chat_history.delete_structured_case",
        _delete_structured_case,
    )
    payload = asyncio.run(
        delete_conversation_endpoint("conv-1", current_user=_request_user())
    )
