...
def test_refresh_qdrant_collection_metrics_updates_gauges(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        metrics_mod, "_default_qdrant_collection", lambda: "sealai_technical_docs"
    )
    monkeypatch.setattr(
        metrics_mod, "_default_qdrant_url", lambda: "http://qdrant:6333"
    )
    ...
    assert (
        metrics_mod.QDRANT_COLLECTION_STATUS.labels(
            collection="sealai_technical_docs"
        )._value.get()
        == 1
    )
    assert (
        metrics_mod.QDRANT_COLLECTION_POINTS.labels(
            collection="sealai_technical_docs"
        )._value.get()
        == 42
    )
    assert (
        metrics_mod.QDRANT_COLLECTION_INDEXED_VECTORS.labels(
            collection="sealai_technical_docs"
        )._value.get()
        == 42
    )
...
def test_track_rag_ingest_records_success_and_timestamp(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda force=False, collection=None: calls.append(bool(force)),
    )

    before = _counter_value(
        metrics_mod.RAG_INGEST_DOCUMENTS_TOTAL, source="paperless", status="indexed"
    )
    ...
    assert (
        metrics_mod.RAG_LAST_SUCCESSFUL_INGEST_TIMESTAMP_SECONDS.labels(
            source="paperless"
        )._value.get()
        > 0
    )
...
def test_track_rag_sync_records_last_run_counts(monkeypatch) -> None:
    monkeypatch.setattr(
        metrics_mod,
        "refresh_qdrant_collection_metrics",
        lambda force=False, collection=None: None,
    )

    before = _counter_value(
        metrics_mod.RAG_SYNC_RUNS_TOTAL, source="paperless", status="success"
    )
    ...
    assert after == before + 1
    assert (
        metrics_mod.RAG_SYNC_LAST_DOCUMENTS.labels(
            source="paperless", result="queued"
        )._value.get()
        == 3
    )
    assert (
        metrics_mod.RAG_SYNC_LAST_DOCUMENTS.labels(
            source="paperless", result="errors"
        )._value.get()
        == 1
    )
