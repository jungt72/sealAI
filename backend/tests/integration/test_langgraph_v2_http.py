...
def test_health_200_no_auth_required() -> None:
    client = _client()
    res = client.get("/api/v1/langgraph/health")
...
def test_state_requires_auth_401() -> None:
    client = _client()
    res = client.get("/api/v1/langgraph/state", params={"thread_id": "default"})
...
def test_confirm_go_requires_auth_401() -> None:
    client = _client()
    res = client.post("/api/v1/langgraph/confirm/go", json={"chat_id": "default", "go": True})
    assert res.status_code == 401
