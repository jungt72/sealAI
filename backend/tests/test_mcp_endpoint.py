...
os.environ.setdefault(
    "POSTGRES_DSN", "postgresql://sealai:secret@localhost:5432/sealai"
)
os.environ.setdefault(
    "postgres_dsn", "postgresql://sealai:secret@localhost:5432/sealai"
)
...
def test_mcp_tools_list_hidden_without_scope() -> None:
    ...
    response = asyncio.run(
        endpoint.handle_mcp_request(request, user=_request_user(scopes=["openid"]))
    )
...
def test_mcp_tools_call_executes_when_scope_is_present(monkeypatch) -> None:
    ...
    def _fake_execute_tool_call(
        *, tool_name: str, arguments: Dict[str, Any], tenant_id: str
    ):
...
def test_mcp_get_available_filters_call_executes_when_scope_is_present(
    monkeypatch,
) -> None:
    endpoint = importlib.import_module("app.api.v1.endpoints.mcp")
    ...
    def _fake_execute_tool_call(
        *, tool_name: str, arguments: Dict[str, Any], tenant_id: str
        ...
        return {
            ...
            "structuredContent": {
                "filters": ["material_code", "additional_metadata.density_kg_m3"]
            },
        }
    ...
    assert response.error is None
    assert (
        "additional_metadata.density_kg_m3"
        in response.result["structuredContent"]["filters"]
    )
