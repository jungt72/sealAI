...
def upgrade() -> None:
    op.add_column(
        ...
        sa.Column(
            ...
            sa.String(),
            nullable=False,
            server_default="not_extracted",
        ),
    )
    op.add_column(
        "rag_documents", sa.Column("extracted_candidates", sa.JSON(), nullable=True)
    )
    op.add_column("rag_documents", sa.Column("evidence_refs", sa.JSON(), nullable=True))
    op.add_column(
        "rag_documents",
        sa.Column(
            "provenance", sa.String(), nullable=False, server_default="documented"
        ),
    )
    op.create_index(
        "ix_rag_documents_extraction_status", "rag_documents", ["extraction_status"]
    )
