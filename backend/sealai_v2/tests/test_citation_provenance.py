...
def test_citation_surfaces_primary_source_not_card_id():
    ...
    assert "FK-ORING-VERPRESSUNG" not in str(
        c
    )  # internal card_id never exposed to the user


def test_citation_fallback_when_no_primary_source_hides_card_id():
    f = GroundingFact(
        text="x", quelle="Fachkarte FK-EPDM (reviewed)", card_id="FK-EPDM", sources=()
    )
    c = citation(f)
...
def test_sources_field_is_L1_neutral_byte_identical_prompt():
    ...
    gf_with = [
        GroundingFact(
            text="t", quelle="q", card_id="c", sources=("Parker", "ISO 3601-2")
        )
    ]
