...
from sealai_v2.material_evidence_ai_review import audit as audit_module
...
def test_ai_review_golden_hash_domains_are_frozen() -> None:
    ...
    assert audit_input.audit_input_sha256 == (
        "4319e960131e7246e7a6176dbf4b1ad8945038f6e6c38616f090a83fe055da25"
    )
    assert report_hash == (
        "2b4331dcec56ed65895941ff895b5824293eeef79bbd5ba4c4d8aaf7ca63662d"
    )
    assert challenge.challenge_id == (
        "mac_7f75df8053c0435286d79344ca2932579d63f7c191482c4067817ff689da965f"
    )
    assert adjudication.adjudication_id == (
        "maa_c3db8d0626994e68870bd60083ad23fd09d142d24c469d3864825aec4aebfcbf"
    )
...
def test_audit_corpus_safety_preflight_blocks_person_name_in_publisher(
    ...
    source = payload.sources[0]
    unsafe_source = AISourceContextV1(
        metadata=replace(source.metadata, publisher=person_name)
    )
    unsafe_payload = replace(payload, sources=(unsafe_source,))
    with pytest.raises(AIReviewValidationError) as exc:
        build_claude_audit_input(AIReviewSnapshotV1.create(BATCH_ID, unsafe_payload))
...
def test_audit_corpus_safety_receipt_binds_publisher_exception_contract(
    monkeypatch,
) -> None:
    payload, _, _ = _payload()
    snapshot = AIReviewSnapshotV1.create(BATCH_ID, payload)
