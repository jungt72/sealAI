...
def test_memory_items_has_every_patch9_column(db_url, column):
    engine = make_engine(db_url)
    cols = {c["name"] for c in inspect(engine).get_columns("v2_memory_items")}
    assert column in cols
...
def test_memory_item_insert_and_query_roundtrip(db_url):
    ...
    with sm() as s:
        ...
        assert row.confidence == 1.0  # Patch 9 default
        assert row.sensitivity == "internal"  # Patch 9 default
        assert row.subject_hash is None
        assert row.supersedes_memory_id is None
        assert row.deprecated_by_memory_id is None
...
def test_memory_item_case_and_project_denormalized_columns_are_independently_nullable(
    ...
    with sm() as s:
        ...
        # Patch 9: the other three denormalized dimensions must independently stay unset too.
...
def test_memory_source_patch9_reference_columns_are_nullable_and_settable(db_url):
    sm = make_sessionmaker(make_engine(db_url))
    with sm() as s:
        s.add(
            V2MemoryItem(
    ...
    with sm() as s:
        row = s.execute(
            select(V2MemorySource).where(V2MemorySource.memory_item_id == "mem-3")
        ).scalar_one()
        assert row.source_ref == "upload-42"
        assert row.message_id == "msg-7"
        assert row.document_id == "doc-9"
        assert row.case_snapshot_id == "case-snap-3"
...
def test_memory_outbox_defaults_to_pending_with_zero_attempts(db_url):
    ...
    with sm() as s:
        s.add(
            V2MemoryOutbox(
                memory_item_id="mem-1",
                tenant_id="tenant-a",
                event_type="upsert",  # Patch 9 rename: operation -> event_type
                created_at="2026-07-03T00:00:00Z",
            )
        )
        s.commit()
    with sm() as s:
        ...
        assert row.target == "qdrant"  # Patch 9 default
        assert row.payload is None  # only the writer (memory_store.py) populates it
        assert row.next_attempt_at is None


def test_memory_outbox_payload_snapshot_roundtrips_as_json(db_url):
    sm = make_sessionmaker(make_engine(db_url))
    with sm() as s:
        s.add(
            V2MemoryOutbox(
                ...
                tenant_id="tenant-a",
                event_type="upsert",
                payload={"id": "mem-1", "content": "prefers metric units"},
                next_attempt_at="2026-07-03T01:00:30Z",
                created_at="2026-07-03T00:00:00Z",
            )
        )
        s.commit()
    with sm() as s:
        ...
        assert row.payload == {"id": "mem-1", "content": "prefers metric units"}
