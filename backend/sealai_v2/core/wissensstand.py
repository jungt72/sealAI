...
``version`` string (fachkarten_seed.json, matrix_seed.json, trap_catalog.json,
...
def compute_wissensstand(
    *,
    ...
    fixed label order (fk, mx, trap, vm). A catalog with no version (not wired for this pipeline —
