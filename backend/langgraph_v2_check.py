...
async def _claim_client_msg_id(*, user_id: str, chat_id: str, client_msg_id: str, tenant_id: str | None = None) -> bool:
    ...
    if client is None:
        return True
    if tenant_id:
        key = f"langgraph_v2:dedup:{tenant_id}:{user_id}:{chat_id}:{client_msg_id}"
    else:
    ...
    *,
    ...
    try:
        ...
        async def _enqueue_frame(frame: bytes, *, allow_slow_notice: bool = True) -> None:
            ...
            if send_slow_notice and queue.maxsize >= 2:
                ...
                if queue.qsize() <= queue.maxsize - 1:
                    ...
                    slow_seq = await sse_broadcast.record_event(user_id=scoped_user_id, tenant_id=scoped_tenant_id,
                        chat_id=req.chat_id,
                        event="slow_client",
        ...
        async def _emit_event(event_name: str, payload: Dict[str, Any]) -> None:
            seq = await sse_broadcast.record_event(user_id=scoped_user_id, tenant_id=scoped_tenant_id,
                chat_id=req.chat_id,
                event=event_name,
                data=payload,)
        ...
        broadcast_queue = await sse_broadcast.subscribe(user_id=scoped_user_id, tenant_id=scoped_tenant_id, chat_id=req.chat_id)
        ...
        if last_seq is not None:
            replay, buffer_miss = await sse_broadcast.replay_after(user_id=scoped_user_id, tenant_id=scoped_tenant_id,
                chat_id=req.chat_id,
                last_seq=last_seq,)
            if buffer_miss:
                await _emit_event(
        ...
        error_seq = await sse_broadcast.record_event(user_id=scoped_user_id, tenant_id=scoped_tenant_id,
            chat_id=req.chat_id,
            event="error",
            data=error_payload,)
        ...
        done_seq = await sse_broadcast.record_event(user_id=scoped_user_id, tenant_id=scoped_tenant_id,
            chat_id=req.chat_id,
            event="done",
            data=done_payload,)
        ...
        if broadcast_queue is not None:
            await sse_broadcast.unsubscribe(user_id=scoped_user_id, tenant_id=scoped_tenant_id,
                chat_id=req.chat_id,
...
async def langgraph_chat_v2_endpoint(
    ...
    if request.client_msg_id:
        claimed = await _claim_client_msg_id(
            ...
            client_msg_id=request.client_msg_id, user_id=scoped_user_id,
        )
        if not claimed:
            raise HTTPException(
                status_code=409,
...
async def patch_parameters(
    ...
    try:
        ...
        await sse_broadcast.broadcast(user_id=scoped_user_id, tenant_id=scoped_tenant_id,
            chat_id=chat_id,
