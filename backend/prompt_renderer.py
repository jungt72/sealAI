...
class PromptRenderer:
    def __init__(
        self, template_dir: str = "/root/sealai/backend/app/services/langgraph/prompts"
    ):
    ...
    def render_template(self, template_name: str, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if "{{" in rendered or "}}" in rendered:
                logger.warning(
                    f"Unvollständige Ersetzung in {template_name}: {rendered[:200]}..."
                )
    ...
    def render_with_validation(
        self, template_name: str, user_input: str, context: Dict[str, Any] = {}
    ) -> str:
        """
...
if __name__ == "__main__":
    ...
    result = renderer.render_with_validation(
        "material_agent.jinja2", "Erkläre PTFE", {}
    )
    print(result)
